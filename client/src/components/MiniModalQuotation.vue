...
const dataSpread = ref(null);

const dataTitle = ref(null);

const activeLayer = ref({ foundation: 0, roof: 0 }); // part_1 or part_2 for foundation quotation

const selectSpecificQuotation = (projectId, dataSource) => {
  const dataToProvide = null;

  switch (projectId) {
    case "danton_shield":
      dataToProvide = dataSource[0];
      break;
    case "merry_clap":
      dataToProvide = dataSource[1];
      break;
    case "dexter_flip":
      dataToProvide = dataSource[2];
      break;
    default:
      throw new Error(
        "There is a problem Selecting Specific Quotation in -- MiniModalQuotation -- component",
      );
  }

  dataTitle.value.activeLayer = activeLayer.value;

  dataSpread.value = dataToProvide;
};
...
const caseToLook = () => {
  ...
  switch (props.quotationInfo.quotationType) {
    case "foundation":
      dataSource = quotationFoundations;
      dataTitle.value.quotationType = "foundation";
      selectSpecificQuotation(props.quotationInfo.projectId, dataSource);
      break;
    case "plumbing":
      dataSource = quotationPlumbing;
      dataTitle.value.quotationType = "plumbing";
      selectSpecificQuotation(props.quotationInfo.projectId, dataSource);
      break;
    case "electricity":
      dataSource = quotationElectricity;
      dataTitle.value.quotationType = "electricity";
      selectSpecificQuotation(props.quotationInfo.projectId, dataSource);
      break;
    case "roofing":
      dataSource = quotationRoofing;
      dataTitle.value.quotationType = "roofing";
      selectSpecificQuotation(props.quotationInfo.projectId, dataSource);
      break;
    default:
      throw new Error(
        "Error Fn *caseToLook* --MiniModalQuotation component-- ",
      );
      break;
  }
};
...
onMounted(async () => {
  if (props.userLanguage) {
    ...
    dataTitle.value.userLanguage = indexLang.val;
  }

  /* initialize *dataSpread* reactive variable */
...
<template>
  <div class="item__quotation w-full h-full">
    ...
    >
      <p>{{ dataSpread?.letterRef }}</p>
      <p>-</p>
      <p v-if="!indexLang.val">Foundation And Structure</p>
      <p v-else></p>
    </div>
    <div class="record__quotation w-full pt-4">
      ...
      <!-- <div
        class="record__delivery w-full h-16 mt-4 px-2 flex flex-row justify-between"
      >
        <p>Delivery Times</p>
